...
% x와 y 데이터 정의 (x의 크기와 y(Q_norm1)의 크기가 같아야 함)
...
figure; % 새 창에 그래프 그리기
line_alpha = 0.2;
scatter(x, y, 10, 'o'); hold on; % 원래 데이터 포인트
scatter(x_fine, y_smooth, 10, 'or', 'MarkerFaceAlpha', line_alpha); % 스플라인 보간 곡선
...
for i = 1:length(hBar)
    ...
    hBar(i).CData =  repmat(colors(i, :), size(hBar(i).YData', 1), 1);
      ...
      if i == 1
        for j = 1:length(x1_adjusted)
          
            for k = 0:0.01:y1(j) 
            
                line([x1_adjusted(j)-0.2, x1_adjusted(j)+0.2], [k, k], 'Color', 'k', 'LineWidth', 0.5); % 검은색 스트라이프
            end
        end
    end
end
...
line_alpha = 0.2;
scatter(x, y, 10, 'o'); hold on; % 원래 데이터 포인트
scatter(x_fine, y_smooth, 10,  'o', 'Color', [0.494117647058824	0.380392156862745	0.282352941176471], 'MarkerFaceAlpha', line_alpha); % 스플라인 보간 곡선

 



% plot([17 100 200 300 417 500], Q_norm2, '-s', 'LineWidth', 0.5);

% 두 번째 그룹
hBar = bar(x2_adjusted, y2, 'stacked', 'BarWidth', barWidth, 'FaceAlpha', 0.8);
for i = 1:length(hBar)
    hBar(i).FaceColor = 'flat'; 
    hBar(i).EdgeColor = [0.5, 0.5, 0.5]; 
    hBar(i).CData =  repmat(colors(i, :), size(hBar(i).YData', 1), 1);
end
...
h = legend({'QC Loss', '1C Loss', 'Loss by LLI', 'Loss by LAMp', 'Loss by R'}, 'Location', 'SouthWest');
